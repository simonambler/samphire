...
if [ "${CONTAINER_CLI}" = "podman" ]; then
  # Initialise the manifest if it does not already exist.
  podman manifest inspect sjambler/samphire >/dev/null 2>&1 || \
    podman manifest create sjambler/samphire

  # Build the production image (webapp stage) and add to the local manifest.
  podman build \
    --platform linux/arm64/v8,linux/amd64/v4 \
    --target webapp \
    --manifest sjambler/samphire \
    "$@" .
else
